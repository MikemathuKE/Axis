...
nk_panel_begin(struct nk_context *ctx, const char *title, enum nk_panel_type panel_type)
{
    ...
    if ((win->flags & NK_WINDOW_MOVABLE) && !(win->flags & NK_WINDOW_ROM)) {
        ...
        int left_mouse_clicked_in_panel;
        ...
        if (left_mouse_down && left_mouse_click_in_cursor && !left_mouse_clicked) {
            ...
            struct nk_rect inner_bound, top_bound, left_bound, right_bound, bottom_bound;
            struct nk_rect selection = nk_dockspace_set_highlight(ctx, &left_bound, &right_bound, &top_bound, &bottom_bound, &inner_bound);
            ...
            nk_fill_rect(out, right_bound, 0, highlightColor);
            nk_fill_rect(out, bottom_bound, 0, highlightColor);
            highlightColor.a = 128;
            if (inner_bound.w != 0 && inner_bound.h != 0)
                nk_fill_rect(out, inner_bound, 0, highlightColor);
            nk_fill_rect(out, selection, 0, highlightColor);
        }
...
nk_panel_end(struct nk_context *ctx)
{
        ...
        window->scrollbar_hiding_timer < NK_SCROLLBAR_HIDING_TIMEOUT)
    {
        ...
        }
        {
            ...
            if (layout->flags & NK_WINDOW_SCALE_LEFT)
                scroll.x = layout->bounds.x + scrollbar_size.x;
            else
    ...
    if (layout->flags & NK_WINDOW_BORDER)
    {
        ...
        //Change cursor if mouse above border
        if ((layout->flags & NK_WINDOW_SCALABLE) && in &&
            !(layout->flags & NK_WINDOW_MINIMIZED) && !(window->flags & NK_WINDOW_ROM)) {

            struct nk_vec2 window_size = style->window.min_size;
            const struct nk_vec2 mouse = ctx->input.mouse.pos;
            struct nk_rect left_border = {b.x-10, b.y, layout->border + 10, b.h};
            struct nk_rect right_border = {b.x + b.w - layout->border, b.y, layout->border + 10, b.h};
            struct nk_rect top_border = {b.x, b.y - layout->border - 12, b.w, layout->border + 10};
            struct nk_rect bottom_border = {b.x, b.y + b.h - layout->border, b.w, layout->border + 10};

            int left_mouse_down = in->mouse.buttons[NK_BUTTON_LEFT].down;

            if ((mouse.x >= left_border.x && mouse.x <= left_border.x + left_border.w) &&
            (mouse.y >= left_border.y && mouse.y <= left_border.y + left_border.h)) {
                int left_mouse_click_in_boundary = nk_input_has_mouse_click_down_in_rect(in,
                    NK_BUTTON_LEFT, left_border, nk_true);

                if (left_mouse_down && left_mouse_click_in_boundary) {
                    float delta_x = in->mouse.delta.x;
                    if (window->bounds.w - delta_x >= window_size.x) {
                        if ((delta_x < 0) || (delta_x > 0)) {
                            window->bounds.x += delta_x;
                            window->bounds.w -= delta_x;
                            left_border.x += delta_x;
                            nk_dockspace_scale_horizontally(ctx, window, delta_x);
                        }
                    }
                    in->mouse.buttons[NK_BUTTON_LEFT].clicked_pos.x = left_border.x + left_border.w/2.0f;
                    in->mouse.buttons[NK_BUTTON_LEFT].clicked_pos.y = left_border.y + left_border.h/2.0f;
                }
                ctx->style.cursor_active = ctx->style.cursors[NK_CURSOR_RESIZE_HORIZONTAL];
            }
            else if ((mouse.x >= right_border.x && mouse.x <= right_border.x + right_border.w) &&
            (mouse.y >= right_border.y && mouse.y <= right_border.y + right_border.h)) {
                int left_mouse_click_in_boundary = nk_input_has_mouse_click_down_in_rect(in,
                    NK_BUTTON_LEFT, right_border, nk_true);
                ...
                if (left_mouse_down && left_mouse_click_in_boundary) {
                    ...
                    if (window->bounds.w + delta_x >= window_size.x) {
                        if ((delta_x < 0) || (delta_x > 0)) {
                            window->bounds.w += delta_x;
                            right_border.x += delta_x;
                            nk_dockspace_scale_horizontally(ctx, window, delta_x);
                        }
                    }
                    in->mouse.buttons[NK_BUTTON_LEFT].clicked_pos.x = right_border.x + right_border.w/2.0f;
                    in->mouse.buttons[NK_BUTTON_LEFT].clicked_pos.y = right_border.y + right_border.h/2.0f;
                }
                ctx->style.cursor_active = ctx->style.cursors[NK_CURSOR_RESIZE_HORIZONTAL];
            } else if ((mouse.x >= top_border.x && mouse.x <= top_border.x + top_border.w) &&
            (mouse.y >= top_border.y && mouse.y <= top_border.y + top_border.h)) {
                int left_mouse_click_in_boundary = nk_input_has_mouse_click_down_in_rect(in,
                    NK_BUTTON_LEFT, top_border, nk_true);

                if (left_mouse_down && left_mouse_click_in_boundary) {
                    float delta_y = in->mouse.delta.y;
                    if (window->bounds.h + delta_y >= window_size.y) {
                        if ((delta_y < 0) || (delta_y > 0)) {
                            window->bounds.y += delta_y;
                            window->bounds.h -= delta_y;
                            top_border.y += delta_y;
                            nk_dockspace_scale_vertically(ctx, window, delta_y);
                        }
                    }
                    in->mouse.buttons[NK_BUTTON_LEFT].clicked_pos.x = top_border.x + top_border.w/2.0f;
                    in->mouse.buttons[NK_BUTTON_LEFT].clicked_pos.y = top_border.y + top_border.h/2.0f;
                }
                ctx->style.cursor_active = ctx->style.cursors[NK_CURSOR_RESIZE_VERTICAL];
            } else if ((mouse.x >= bottom_border.x && mouse.x <= bottom_border.x + bottom_border.w) &&
            (mouse.y >= bottom_border.y && mouse.y <= bottom_border.y + bottom_border.h)) {
                int left_mouse_click_in_boundary = nk_input_has_mouse_click_down_in_rect(in,
                    NK_BUTTON_LEFT, bottom_border, nk_true);
                ...
                if (left_mouse_down && left_mouse_click_in_boundary) {
                    ...
                    if (window->bounds.h + delta_y >= window_size.y) {
                        if ((delta_y < 0) || (delta_y > 0)) {
                            window->bounds.h += delta_y;
                            bottom_border.y += delta_y;
                            nk_dockspace_scale_vertically(ctx, window, delta_y);
                        }
                    }
                    in->mouse.buttons[NK_BUTTON_LEFT].clicked_pos.x = bottom_border.x + bottom_border.w/2.0f;
                    in->mouse.buttons[NK_BUTTON_LEFT].clicked_pos.y = bottom_border.y + bottom_border.h/2.0f;
                }
                ctx->style.cursor_active = ctx->style.cursors[NK_CURSOR_RESIZE_VERTICAL];
            }
        }
